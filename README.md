...
- CodeRabbit profile: start from [supervisor.config.coderabbit.json](./supervisor.config.coderabbit.json), install CodeRabbit, and verify review activity arrives from `coderabbitai` or `coderabbitai[bot]`. The shipped profile also waits up to 30 minutes after a CodeRabbit `Rate limit exceeded` warning before continuing, shows `configured_bot_initial_grace_wait ... pause_reason=awaiting_initial_provider_activity ...` right after checks turn green, reuses that same wait label with `pause_reason=awaiting_fresh_provider_review_after_draft_skip ...` when a prior signal was only a draft skip and the PR later becomes ready for review, and finally shows `configured_bot_settled_wait status=active provider=coderabbit pause_reason=recent_current_head_observation ... wait_until=...` after a fresh current-head observation.
