...
- `operator_action action=resolve_stale_review_bot`: code or CI is green, but stale configured-bot review thread metadata still blocks the tracked PR; inspect the exact thread URL reported by `stale_review_bot_remediation` and the counts or suppression reason in `stale_review_bot_thread_diagnostics`, then resolve it or leave a manual note without changing merge policy. A top-level Codex Connector success comment does not override unresolved unverified inline P1/P2 findings.
