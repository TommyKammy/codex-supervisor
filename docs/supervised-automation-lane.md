...
Operator-facing state is a trust surface. It tells the operator which authority boundary currently owns progress, which evidence proves that conclusion, and what action is allowed next. Derived status text for live operator surfaces such as `status` and `explain` should be recalculated from authoritative lifecycle records, fresh GitHub facts, and the issue journal when those surfaces are produced, so it remains a reconciliation target instead of becoming a separate source of truth. Persisted PR status comments are snapshots that may only be republished on head or signature changes and must not be treated as authoritative lifecycle state.

The portable contract artifact is [Supervised automation state machine](./supervised-automation-state-machine.schema.json). It maps the public lane states below to the internal `RunState` values in `src/core/types.ts`, and it binds each public state to the published operator action tokens in [Operator actions](./operator-actions.schema.json). Public names are intentionally broader than some implementation phases: for example `reproducing`, `implementing`, and `local_review` are public `running`; `draft_pr`, `pr_open`, and `addressing_review` are public `waiting_review`; and `ready_to_merge` is public `merging`.
...
| `failed` | A Codex turn, recovery path, or repeated failure policy ended in a failed local lifecycle state instead of a named blocked prerequisite. | Last failure kind, failure signature, last error, failure context, issue journal, recovery diagnostics, and any tracked PR facts that prove whether recovery can resume safely. | Shared boundary: supervisor records and may route only explicitly recoverable failures; operator judgment owns requeue, manual repair, close, or follow-up decisions when recovery evidence is missing. | Run `explain` or `status --why`, inspect the failure evidence, requeue only after the prerequisite is real, or keep the issue failed/manual-review until a safe recovery path is explicit. |
