...
実装中は authoritative と derived の state 選択も明示してください。authoritative な lifecycle record と summary、timeline projection、badge などの operator-facing な convenience surface が食い違う時は、authoritative 側を優先します。
anchored context や lineage を推測だけで広げないでください。advisory、assistant、detail surface を組み立てる時は、direct authoritative linkage を sibling 由来や indirect lineage より優先します。
