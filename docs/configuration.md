...
| --- | --- | --- | --- | --- |
| `trusted_repo_and_authors` | `operator_gated` | Safe | The repo and GitHub authors are trusted, but execution still stays behind an operator gate. Good first production posture when you want explicit review before autonomous turns. | `trust_mode=trusted_repo_and_authors`, `execution_safety_mode=operator_gated`; no unsandboxed warning. |
