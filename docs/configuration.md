...
Verified current-head repair residue can clear or resolve stale Connector thread residue, but it is not treated as a current-head Connector no-major signal for the final auto-merge guard. When the Codex Connector auto-merge path is enabled, the guard still requires an actual current-head Connector success/no-major observation before merge.
