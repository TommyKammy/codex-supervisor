...
export function renderDoctorReport(diagnostics: DoctorDiagnostics): string {
  ...
  return [
    ...
    `doctor_loop_runtime state=${loopRuntime.state} host_mode=${loopRuntime.hostMode} run_mode=${loopRuntime.runMode ?? "unknown"} marker_path=${sanitizeDoctorValue(loopRuntimeMarkerPath)} config_path=${sanitizeDoctorValue(loopRuntimeConfigPath ?? "none")} state_file=${sanitizeDoctorValue(loopRuntimeStateFile)} pid=${loopRuntime.pid === null ? "none" : String(loopRuntime.pid)} started_at=${loopRuntime.startedAt ?? "none"} ownership_confidence=${loopRuntimeOwnershipConfidence} detail=${sanitizeDoctorValue(loopRuntime.detail ?? "none")}`,
