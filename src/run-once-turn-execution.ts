...
export function renderCodexExecutionSummary(
  ...
  const routingSummary = [
    ...
    `requested_model=${routing.requestedModel ?? (routing.modelStrategy === "inherit" ? "inherit" : routing.model ?? "inherit")}`,
    `effective_model=${routing.effectiveModel ?? routing.model ?? "unresolved"}`,
    `model_route_source=${routing.modelRouteSource ?? "unavailable"}`,
    `model_fallback_source=${routing.modelFallbackSource ?? "none"}`,
    `model_capability_source=${routing.modelCapabilitySource ?? "unavailable"}`,
    `model_capability_fallback_reason=${routing.modelCapabilityFallbackReason ?? "none"}`,
