...
- Added tracked-PR host-local CI blocker diagnostics to status and doctor so operators can see the local blocker even when GitHub checks are green, including failure class, remediation target, current head, and an explicit workspace-preparation gap when toolchain prerequisites are missing. Verified with focused tests and `npm run build`, committed as `67aa478`, and opened draft PR #1263.
...
- Next exact step: Monitor draft PR #1263 and address any CI or review feedback.
